"""V4f: Biologically realistic limonene odor experiment.
...
Replaces the synthetic random sparse odor with a biologically grounded KC
activation pattern for **limonene**, derived from the DOOr database OR
response profile and real FlyWire PN→KC connectivity (or a principled fallback).

Produces 8 separate publication-ready figures:
  1. Training vs post-training MBON rates
  2. Forward conditioning weight-change heatmap
  3. Backward conditioning weight-change heatmap
  4. Forward conditioning per-MBON bar chart
  5. Backward conditioning per-MBON bar chart
  6. Training dynamics (5×2 grid)
  7. Preference summary bar chart
  8. Side-by-side weight heatmaps
"""
...
def run_v4f():
    print("=" * 65)
    print("V4f: Limonene Odor — Gkanias DPR + Bennett KC-gating + Huang Dynamics")
    print("=" * 65)
    ...
    print(f"MBONs: {len(approach_idx)} approach, {len(avoid_idx)} avoid, "
    ...
    # Compute preference values for later figures
    app_vals, avd_vals = [], []
    ...
    # ====================================================================
    # Run a fresh forward training trial recording per-MBON rates
    ...
    # Top-left: During training — group means
    ax = axes1[0, 0]
    ...
    # Top-right: During training — individual MBONs
    ax = axes1[0, 1]
    ...
    # Bottom-left: Post-training test — group means + naive overlay
    ax = axes1[1, 0]
    ...
    fig1.suptitle('Training vs Post-Training MBON Activity — Limonene (Forward Conditioning)\n'
                  'Green = approach MBONs, Red = avoid MBONs',
                  fontweight='bold', fontsize=12)
    fig1.tight_layout()
    fig1.savefig('results/v4f_train_vs_test.png', dpi=150, bbox_inches='tight')
    plt.close(fig1)
    print("\n--- Figures ---")
    print("  Saved: results/v4f_train_vs_test.png")
    ...
    plt.colorbar(im2, ax=ax2, label='ΔW (weight change)')
    ax2.set_xlabel(f'Odor-active KCs ({len(odor_kcs_sorted)} of {n_kc})')
    ax2.set_ylabel('MBON')
    ax2.set_title('Forward Conditioning — KC→MBON Weight Change (Limonene)',
                  fontweight='bold')
    ...
    fig2.tight_layout()
    fig2.savefig('results/v4f_dw_heatmap_forward.png', dpi=150, bbox_inches='tight')
    plt.close(fig2)
    print("  Saved: results/v4f_dw_heatmap_forward.png")
    ...
    im3 = ax3.imshow(dW_bwd[:, odor_kcs_sorted], aspect='auto', cmap='RdBu_r',
                     vmin=-vmax, vmax=vmax, interpolation='nearest')
    plt.colorbar(im3, ax=ax3, label='ΔW (weight change)')
    ax3.set_xlabel(f'Odor-active KCs ({len(odor_kcs_sorted)} of {n_kc})')
    ax3.set_ylabel('MBON')
    ax3.set_title('Backward Conditioning — KC→MBON Weight Change (Limonene)',
                  fontweight='bold')
    ...
    for ticklabel, color in zip(ax3.get_yticklabels(), ycolors):
        ticklabel.set_color(color)
    fig3.tight_layout()
    fig3.savefig('results/v4f_dw_heatmap_backward.png', dpi=150, bbox_inches='tight')
    plt.close(fig3)
    print("  Saved: results/v4f_dw_heatmap_backward.png")
    ...
    ax4.bar(x_mbon, mean_dW_fwd, color='steelblue', alpha=0.8)
    ...
    ax4.set_ylabel('Mean ΔW (over odor-active KCs)')
    ax4.set_title('Forward Conditioning — Per-MBON Mean Weight Change (Limonene)',
                  fontweight='bold')
    ax4.grid(axis='y', alpha=0.3)
    fig4.tight_layout()
    fig4.savefig('results/v4f_dw_barplot_forward.png', dpi=150, bbox_inches='tight')
    plt.close(fig4)
    print("  Saved: results/v4f_dw_barplot_forward.png")

    # ====================================================================
    # FIGURE 5 — Backward conditioning per-MBON bar chart
    # ====================================================================
    fig5, ax5 = plt.subplots(figsize=(16, 5))
    ax5.bar(x_mbon, mean_dW_bwd, color='firebrick', alpha=0.8)
    ...
    ax5.set_ylabel('Mean ΔW (over odor-active KCs)')
    ax5.set_title('Backward Conditioning — Per-MBON Mean Weight Change (Limonene)',
                  fontweight='bold')
    ax5.grid(axis='y', alpha=0.3)
    fig5.tight_layout()
    fig5.savefig('results/v4f_dw_barplot_backward.png', dpi=150, bbox_inches='tight')
    plt.close(fig5)
    print("  Saved: results/v4f_dw_barplot_backward.png")
    ...
    for col, (rec, cond_label, cs_on, cs_off, us_on, us_off) in enumerate([
        ...
        ax.set_title(f'{cond_label} — Limonene Gkanias DPR',
                     fontweight='bold', fontsize=12)
        ax.legend(fontsize=8, loc='upper right')
        ...
        ax = axes6[4, col]
        ax.plot(t, rec['mean_delta'], color='navy', lw=1.5, label='δ = D_▽ − D_△ (slow − fast)')
        ax_tw = ax.twinx()
    ...
    fig6.suptitle('Training Dynamics — Limonene Gkanias DPR Model (Trial 1)\n'
                  'Original Gkanias et al. (2022) rule with Bennett KC-gating',
                  fontweight='bold', fontsize=12)
    fig6.tight_layout()
    fig6.savefig('results/v4f_dynamics.png', dpi=150, bbox_inches='tight')
    plt.close(fig6)
    print("  Saved: results/v4f_dynamics.png")
    ...
    ax7.set_title('Limonene — Preference Summary\n'
                  '(solid = approach, hatched = avoid)', fontweight='bold')
    ax7.legend(fontsize=10)
    ax7.grid(axis='y', alpha=0.3)
    fig7.tight_layout()
    fig7.savefig('results/v4f_preference.png', dpi=150, bbox_inches='tight')
    plt.close(fig7)
    print("  Saved: results/v4f_preference.png")
    ...
    fig8.suptitle(f'KC→MBON Synaptic Weight Changes After {n_trials} Trials — Limonene\n'
                  'Gkanias DPR Model (green = approach, red = avoid)',
                  fontweight='bold', fontsize=12)
    fig8.tight_layout()
    fig8.savefig('results/v4f_weight_heatmaps.png', dpi=150, bbox_inches='tight')
    plt.close(fig8)
    print("  Saved: results/v4f_weight_heatmaps.png")

    # ====================================================================
    # Numerical summary
    # ====================================================================
    print("\n" + "-" * 50)
    print("LIMONENE GKANIAS DPR MODEL SUMMARY")
    print("-" * 50)
